...
class TRANSPORT:
  """Build the transport matrix, i.e., everything except the external source for SI
  and GMRES."""

  def __init__(self, dof_handler, quadrature, solver_type, prec) :
...
#----------------------------------------------------------------------------#
  ...
  def Compute_largest_eigenvalue(self,lambdas,sign=None) :
    ...
    if self.preconditioner==False :
      ...
      mip = MIP.MIP(self.dof_handler, self.quad, lambda_x, lambda_y)
      mip.Build_matrix()
      mip.Invert()
...
# Direction alias
      ...
      for cell in self.dof_handler.grid :
        pos = 4*cell.fe.fe_id+4*self.dof_handler.n_cells*idir
        if self.absolute_phase==False :
          self.L[pos:pos+4,pos:pos+4] += -omega_x*cell.fe.x_grad_matrix-\
              omega_y*cell.fe.y_grad_matrix+cell.sigma_t*cell.fe.mass_matrix
        else :  
          self.Absolute_phase(cell.fe)
          self.L[pos:pos+4,pos:pos+4] += np.dot(-omega_x*cell.fe.x_grad_matrix-\
              omega_y*cell.fe.y_grad_matrix+cell.sigma_t*cell.fe.mass_matrix,\
              self.phase)

# Compute the normal of each side
        x = cell.fe.y[1]-cell.fe.y[0]
        y = cell.fe.x[0]-cell.fe.x[1]
        norm = np.sqrt(x**2+y**2)
        b_normal = np.array([x/norm,y/norm])
        x = cell.fe.y[2]-cell.fe.y[1]
        y = cell.fe.x[1]-cell.fe.x[2]
        norm = np.sqrt(x**2+y**2)
        r_normal = np.array([x/norm,y/norm])
        x = cell.fe.y[3]-cell.fe.y[2]
        y = cell.fe.x[2]-cell.fe.x[3]
        norm = np.sqrt(x**2+y**2)
        t_normal = np.array([x/norm,y/norm])
        x = cell.fe.y[0]-cell.fe.y[3]
        y = cell.fe.x[3]-cell.fe.x[0]
        norm = np.sqrt(x**2+y**2)
...
# Upwind
        if n_dot_omega<0.0 :
          if cell.fe.y[0]==self.dof_handler.bottom :
            ...
            if self.absolute_phase==False :
              self.Phase(cell.fe,self.dof_handler.grid[cell.fe.fe_id+\
                  (self.dof_handler.ny_cells-1)*self.dof_handler.nx_cells],
                  "bottom")
            else :
              self.Absolute_phase(self.dof_handler.grid[cell.fe.fe_id+\
                  (self.dof_handler.ny_cells-1)*self.dof_handler.nx_cells].fe,
                  cell.fe,"bottom")
          else :
# This phase matrix is the identity
            ...
            if self.absolute_phase==False :
              ...
              self.Absolute_phase(self.dof_handler.grid[cell.fe.fe_id-\
                  self.dof_handler.nx_cells].fe)
          self.L[pos:pos+4,pos+offset:pos+offset+4] += n_dot_omega*\
              np.dot(cell.fe.bottom_up,self.phase)
# Downwind 
        else :
          if self.absolute_phase==False :
            self.L[pos:pos+4,pos:pos+4] += n_dot_omega*cell.fe.bottom_down
          else :  
            self.Absolute_phase(cell.fe)
            self.L[pos:pos+4,pos:pos+4] += n_dot_omega*np.dot(
                cell.fe.bottom_down,self.phase)
        ...
        if n_dot_omega<0.0 :
          if cell.fe.x[1]==self.dof_handler.right :
            ...
            if self.absolute_phase==False :
              ...
              self.Absolute_phase(self.dof_handler.grid[cell.fe.fe_id-\
                  (self.dof_handler.nx_cells-1)].fe,cell.fe,"right")
          else :
# This phase matrix is the identity
            offset = 4
            if self.absolute_phase==False :
              self.Phase(cell.fe,self.dof_handler.grid[cell.fe.fe_id+1],"right")
            else :
              self.Absolute_phase(self.dof_handler.grid[cell.fe.fe_id+1])
          self.L[pos:pos+4,pos+offset:pos+offset+4] += n_dot_omega*\
              np.dot(cell.fe.right_up,self.phase)
# Downwind
        else :
          if self.absolute_phase==False :
            self.L[pos:pos+4,pos:pos+4] += n_dot_omega*cell.fe.right_down
          else :
            self.Absolute_phase(cell.fe)
            self.L[pos:pos+4,pos:pos+4] += n_dot_omega*np.dot(
                cell.fe.right_down,self.phase)

# Top term
        n_dot_omega = np.dot(omega,t_normal)  
# Upwind
        if n_dot_omega<0.0 :
          if cell.fe.y[3]==self.dof_handler.top :
            offset = -4*self.dof_handler.nx_cells*\
                (self.dof_handler.ny_cells-1)
            if self.absolute_phase==False :
              self.Phase(cell.fe,self.dof_handler.grid[cell.fe.fe_id-\
                  (self.dof_handler.ny_cells-1)*self.dof_handler.nx_cells].fe,"top")
            else :
              self.Absolute_phase(self.dof_handler.grid[cell.fe.fe_id-\
                  (self.dof_handler.ny_cells-1)*self.dof_handler.nx_cells].fe,
                  cell.fe,"top")
...
# This phase matrix is the identity
            ...
            if self.absolute_phase==False :
              ...
              self.Absolute_phase(self.dof_handler.grid[cell.fe.fe_id+\
                  self.dof_handler.nx_cells].fe)
          self.L[pos:pos+4,pos+offset:pos+offset+4] += n_dot_omega*\
              np.dot(cell.fe.top_up,self.phase)
# Downwind 
        else :
          if self.absolute_phase==False :
            self.L[pos:pos+4,pos:pos+4] += n_dot_omega*cell.fe.top_down
          else :
            self.Absolute_phase(cell.fe)
            self.L[pos:pos+4,pos:pos+4] += n_dot_omega*np.dot(cell.fe.top_down,
                self.phase)
...
# Upwind
        if n_dot_omega<0.0 :
          if cell.fe.x[0]==self.dof_handler.left :
            ...
            if self.absolute_phase==False :
              self.Phase(cell.fe,self.dof_handler.grid[cell.fe.fe_id+\
                  (self.dof_handler.nx_cells-1)].fe,"left")
            else :
              self.Absolute_phase(self.dof_handler.grid[cell.fe.fe_id+\
                  (self.dof_handler.nx_cells-1)].fe,cell.fe,"left")
          else :
# This phase matrix is the identity
            offset = -4
            if self.absolute_phase==False :
              self.Phase(cell.fe,self.dof_handler.grid[cell.fe.fe_id-1],"left")
            else :
              self.Absolute_phase(self.dof_handler.grid[cell.fe.fe_id-1].fe)
          self.L[pos:pos+4,pos+offset:pos+offset+4] += n_dot_omega*\
              np.dot(cell.fe.left_up,self.phase)
# Downwind
        else :
          if self.absolute_phase==False :
            self.L[pos:pos+4,pos:pos+4] += n_dot_omega*cell.fe.left_down
          else :
            self.Absolute_phase(cell.fe)
            self.L[pos:pos+4,pos:pos+4] += n_dot_omega*np.dot(cell.fe.left_down,
                self.phase)
    ...
    for cell in self.dof_handler.grid :
      if self.absolute_phase==False :
       for mom in range(0, self.quad.n_mom) :
         pos = 4*mom+4*cell.fe.fe_id*self.quad.n_mom
         matrix[pos:pos+4,pos:pos+4] = cell.sigma_s[mom]*cell.fe.mass_matrix
      else :
        self.Absolute_phase(cell.fe)
        for mom in range(0, self.quad.n_mom) :
          pos = 4*mom+4*cell.fe.fe_id*self.quad.n_mom
          matrix[pos:pos+4,pos:pos+4] = cell.sigma_s[mom]*\
...
#----------------------------------------------------------------------------#
  ...
  def Phase(self,cell_1,cell_2,side) :
    ...
    if side=="bottom" :
      ...
      raise RuntimeError('This side does not exist')
